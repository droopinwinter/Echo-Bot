...
def score_trend(data):
    ...
    for j in range(10,len(data)):
        try:      
            ...
            a=[ data.at[j,'date'],data.at[j,'close'], xema, xmacd, TrnSlop.round(1), 0.0, 0, 0.0, 0 , 0]
            xapka = pd.concat([xapka, pd.DataFrame([a])], ignore_index=True)                
        except Exception as errMsg: 
            print('每K線計算發生錯誤-', str(data.at[j,'date']) , errMsg)                   

    xapka.columns = ["date", "close", "xema", "xmacd", "TrnSlop", "HoldPrice", "sell", "profit","buySig", "SellSig"] 
    return xapka
    '''
