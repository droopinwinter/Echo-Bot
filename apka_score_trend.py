...
def score_trend(data):
    ...
    for j in range(10,len(data)):
        try:        
            ...
            if  data.at[j, 'MACD_d'] > data.at[j, 'signal_d']:
                ...
                xmacd =0
            a=[ data.at[j,'date'],data.at[j,'close'], xema, xmacd, 0, 0, 0.0]
            xapka = pd.concat([xapka, pd.DataFrame([a])], ignore_index=True)                
        except Exception as errMsg: 
            print('每K線計算發生錯誤-', str(data.at[j,'date']) , errMsg)                   

    xapka.columns = ["date", "close", "xema", "xmacd", "buy", "sell", "profit"] 
    ...
    for i in range(1,len(xapka)):
        if xapka.at[i,"xmacd"] ==2 and xapka.at[i-1,"xmacd"]<2:
            sub2.text(xapka.at[i, "date"], 1,str(xapka.at[i, "date"])[5:10],color='green')
            xapka.at[i,"buy"] =1
            buy = xapka.at[i,"close"]
        elif xapka.at[i,"xmacd"] <1 and xapka.at[i-1,"xmacd"]==1:
