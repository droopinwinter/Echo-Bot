...
def TotProfit(xapka, Ticker):
    ...
    def ToBuy(i, xtred):
        ...
        if prebuy ==0:
            ...
            #    return 11
            #elif OscSum <= -5 and xmacd >=4 and xema >=4 and xTrnSlop<=0:
            #    return -11            
            elif  yema > xapka.at[i-1,"yema"]  and yema >=1 and xema >0 and xTrnSlop>=0:
                return yema                          
            elif yema < xapka.at[i-1,"yema"] and yema <=1 and OscSum <2 and xema <0  and xTrnSlop<0:
                return yema
    ...
    def ToSell(i, xtred):
        ...
        if   Prebuy == 10 and (xmacd == 4 or  (OscSum<=-1 and xmacd <0) )  :
            ...
            return Prebuy        
        elif Prebuy < 0 and Prebuy > -10 and ( OscSum>=4 or yema > xapka.at[i-1,"yema"]) and xTrnSlop>=0:
            return Prebuy
        elif Prebuy > 0   and Prebuy < 10 and yema < xapka.at[i-1,"yema"] and xTrnSlop<0: #and Prebuy == xapka.at[i-1,"yema"]
            return Prebuy
        elif Prebuy ==20  and (OscSum <=-4 or (xmacd < xapka.at[i-1,"xmacd"] )) :     
            return Prebuy
        #elif Prebuy ==-20  and (OscSum >=4 or (xmacd > xapka.at[i-1,"xmacd"] )) :     
        #    return Prebuy        
        else:
