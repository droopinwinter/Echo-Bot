...
try:
    ...
    conn = pymssql.connect(host="192.9.12.226:1433", user='sa', password='abc123', database='analsy',charset='GBK')
    cursor = conn.cursor()    
...
for i in Ticker.Stock:
    ...
    try:
        sql4 = \
            ...
            "	) FROM [Stock].[dbo]."+'RowDay_'+ i.strip()+\
        ...
        sql1 =  "DELETE [analsy].[dbo].AnalysDay_"+ i.strip()+" WHERE date >= '2022-10-11 00:00:00.000' "
        sql7 =  "DELETE [apka].[dbo].ApkaDay_"+ i.strip()+" WHERE date >= '2022-10-11 00:00:00.000' "
        cursor.execute(sql5)
        #cursor.execute(sql6)
        conn.commit()
    except Exception as errMsg:                   # 如果 try 的內容發生錯誤，就執行 except 裡的內容
        print('資料庫刪重複＿發生錯誤-'+i.strip() , errMsg)     